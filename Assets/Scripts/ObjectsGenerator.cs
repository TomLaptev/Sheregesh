    ...
    public void CreateObjects()
    {
        ...
        if (RoadGenerator.flagDestroyRoads)
        {
            ...
            for (var i = 0; i < _numberBarriers; i++)
            {
                ...
                goBarrier.transform.eulerAngles = new Vector3(0, Random.Range(-5, 6) * 30, 0);
                barriers.Add(goBarrier); 
                for (var j = 1; j <= i; j++)
                {
                      if ( Mathf.Abs(barriers[barriers.Count -1].transform.position.x - barriers[barriers.Count -1-j].transform.position.x) <= 1.5f  &&
                    Mathf.Abs(barriers[barriers.Count -1].transform.position.z - barriers[barriers.Count -1-j].transform.position.z) <= 3f)  
                    {
                       barriers[barriers.Count -1].SetActive(false);
        ...
        else if (RoadGenerator.currentSpeed == 0)
        {
            for (var i = 0; i < RoadGenerator.maxRoadTiles; i++)
            {
                if (i == 0)
                {
                    posCoin = RoadGenerator.roads[0].transform.position + new Vector3(Random.Range(-2, 3) * 1.5f, 1.5f, Random.Range(1, 3) * 4);
                }
                else posCoin = RoadGenerator.roads[i].transform.position + new Vector3(Random.Range(-2, 3) * 1.5f, 1.5f, Random.Range(-2, 3) * 4);
